...
export default function DpoPageIsland({ sections, device }: Props) {
    ...
    const desktopComponent = (<div className="flex justify-center px-10 lg:px-0 bg-white">
            <div className="lg:max-w-[1400px] w-full pt-12 pb-16">
                <div className="flex flex-col lg:flex-row gap-10 lg:gap-12">
                    <div className="lg:w-1/4 flex flex-col pt-8 lg:sticky lg:top-8 lg:h-full">
                        {sections.map((section, index) => (<div key={index} className="flex gap-7 justify-between pb-3 mb-14 border-b border-b-black border-opacity-15 cursor-pointer pr-9" onClick={() => handleSectionClick(index)}>
                                <span className={`text-sm ${activeSection.value === index
                ? "text-orange1"
                : "text-black text-opacity-40"} font-sora`} dangerouslySetInnerHTML={{
                __html: section.sectionName,
            }}/>
                                <Image src={`${activeSection.value === index
                ? "/Site/orange-arrow-right.svg"
                : "/Site/gray-arrow-right.svg"}`} alt="Arrow Right"/>
                            </div>))}
                    </div>

                    <div className="lg:w-3/4 flex flex-col gap-14">
                        {sections.map((section, index) => (<div key={index} ref={(el) => {
                if (el) {
    ...
    const mobileComponent = (<div className="flex justify-center px-10 lg:px-0 bg-white">
            <div className="lg:max-w-[1400px] w-full pt-12 pb-16">
                <div className="flex flex-col lg:flex-row gap-10 lg:gap-12">
                    <div className="flex flex-col pt-8 lg:sticky lg:top-8 lg:h-full">
                        {sections.map((section, index) => (<div key={index} className="flex flex-col" onClick={() => {
                ...
                handleSectionClick(index); // Only handle click here, no scroll needed
                                ...
                                <div className="transition-all duration-300 flex gap-2 justify-between pb-3 mb-5 border-b border-b-black border-opacity-15 cursor-pointer pr-9">
                                    <span className={`${expandedSection.value === index
                ? "text-orange1"
                : "text-black text-opacity-40"} font-sora`} dangerouslySetInnerHTML={{
                __html: section.sectionName,
            }}/>
                                    <Image src={`${expandedSection.value === index
                ? "/Site/orange-arrow-down.svg"
                : "/Site/gray-arrow-right.svg"}`} alt="Arrow Right"/>
                                </div>
                                <div className={`overflow-hidden transition-[height] duration-500 ease-in-out ${expandedSection.value === index
                ? "h-auto"
                : "h-0"}`} style={{
