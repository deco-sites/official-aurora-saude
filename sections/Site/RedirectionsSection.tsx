...
export default function RedirectionsSection({ desktopTitle, mobileTitle, desktopText, mobileText, device }: RedirectionsSectionProps) {
    return (<>
            <div className="bg-inherit flex flex-col gap-20 px-10 pt-10 pb-20 lg:px-52 lg:pt-32 lg:pb-44">
                <div className="flex flex-col gap-4 lg:flex-row justify-between">
                    <span className="text-yellow text-5xl font-sora font-semibold lg:w-2/3 leading-tight" dangerouslySetInnerHTML={{
            __html: device === "desktop"
                ? desktopTitle
                : mobileTitle,
        }}>
                    </span>
                    <span className="text-white lg:w-1/3" dangerouslySetInnerHTML={{
            __html: device === "desktop"
                ? desktopText
                : mobileText,
        }}>
                ...
                <div className="flex flex-col gap-4 lg:flex-row justify-between">
                    <a href="#startJourney">
                        <button className="w-full bg-white rounded-full py-6 px-10 text-pink1 flex justify-between gap-9 font-medium lg:font-normal whitespace-nowrap">
                            Jornada de Cuidado
                            <Icon class="h-auto" id="PinkArrowDown" strokeWidth={1} size={20}/>
                        </button>
                    </a>
                    <a href="#appSection">
                        <button className="w-full bg-white rounded-full py-6 px-10 text-pink1 flex justify-between gap-9 font-medium lg:font-normal whitespace-nowrap">
